...
function useForm(fields) {
    const [fieldStates, setFieldStates] = React.useState(fields.reduce((obj, cur) => (Object.assign(Object.assign({}, obj), { [cur.key]: { value: '', hasError: validate('', ...cur.validators) } })), {}));
    ...
    const form = fields.map(({ key, inputProps, validators }) => React__default.createElement(FormField, Object.assign({}, inputProps, { key: key, onValidationChange: (value) => validationChangeHandler(key, value), onChange: (event) => valueChangeHandler(key, event.target.value), value: fieldStates[key].value, validators: validators })));
    const formValidate = () => {
